...
public class PainelFuncionarios extends JPanel implements TableModelListener
{
		...
		try {
			...
			while(pega.next())
			{
				if(pega.getInt("level") < 3)
				{
					...
					funcionarios.add(new Funcionario(pega.getInt("id"), pega.getString("username"), 
	...
	public void atualizaTodosFuncionarios() {
		
		PainelVendaRapida.getInstance().atualizaFuncionarios(getTodosFuncionarios());
		PainelVendaMesa.getInstance().atualizaFuncionarios(getTodosFuncionarios());
	}
